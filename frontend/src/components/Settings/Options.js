...
export default function Options(props) {
  ...
  useEffect(() => {
    if (Array.isArray(settings) && settings.length) {
      ...
      const mercadoPagoToken = settings.find((s) => s.key === "mercado_pago_access_token");
      if (mercadoPagoToken) {
        setMercadoPagoAccessToken(mercadoPagoToken.value);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settings]);
  ...
  async function handleChangeMercadoPago(value) {
        ...
        <Grid xs={12} sm={12} md={12} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              onChange={async (e) => {
                handleChangeMercadoPago(e.target.value);
              }}
            >
            </TextField>
            <FormHelperText>
              {loadingMercadoPago && "Atualizando..."}
            </FormHelperText>
          </FormControl>
        </Grid>
      </Grid>
