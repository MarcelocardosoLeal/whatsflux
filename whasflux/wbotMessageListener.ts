...
const handleMessage = async (
  ...
  // Fix: Treat LID (Linked Device ID) as mapped JID to ensure n8n/webhooks receive the real phone number
  if (msg.key.remoteJid && msg.key.remoteJid.includes("@lid")) {
    const contact = (wbot.store as any)?.contacts?.[msg.key.remoteJid];
    if (contact && contact.id && contact.id.includes("s.whatsapp.net")) {
      msg.key.remoteJid = contact.id;
      msg.participant = contact.id;
      msg.key.participant = contact.id;
    }
  }
